...
rsync -e "ssh -p $sshport -o StrictHostKeyChecking=no" -uvr --include="*.py" --exclude "**/torch_compile_debug/" --exclude "**/.*" --exclude "**/res_cache/" --exclude="**/__pycache__"  --include="*/" --include "*.txt" --include "*.bash" --include "*.sh" --include "*.csv" --include "*.json" --include "*.md" --exclude="*"  $username@$hostname:shared/tests/ $HOME/shared/tests/ & \
