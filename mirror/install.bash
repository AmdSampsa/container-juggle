...
if [ "$TOOLS" = true ]; then
  ...
  docker exec $userflag $container_name /bin/sh -c '
      (apt-get update && 
      apt-get install -y emacs less colorized-logs silversearcher-ag tree dialog psmisc ccache ssh iputils-ping psmisc jq pkg-config liblzma-dev) || 
      (yum install -y emacs less the_silver_searcher tree dialog psmisc ccache openssh iputils psmisc jq pkgconfig liblzma-devel)
  '
  docker exec $userflag $container_name /bin/sh -c '
