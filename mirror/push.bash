...
rsync -e "ssh -p $sshport -o StrictHostKeyChecking=no" -uvr --exclude "**/.*" --exclude "SAVED" --include="*.py" --include "*.txt" --include "*.csv" --include "*.json" --include "*.md" --include "*.bash" --include "*.sh" --include="*/" --exclude="*" $HOME/shared/tests/ $username@$hostname:shared/tests/ & \
